...
describe("preparePublishRequests", () => {
  ...
  it("routes Lunary OG media through the convert-media-url proxy", async () => {
    ...
    const encodedUrl = Buffer.from(lunaryUrl, "utf8").toString("base64url");
    const proxiedUrl = `${MEDIA_PROXY_BASE_URL}/api/convert-media-url?u=${encodedUrl}&format=png`;
    ...
    expect(instagramRequest?.postData.mediaUrls).toEqual([
      proxiedUrl,
      `${MEDIA_PROXY_BASE_URL}/api/convert-media-url?u=${Buffer.from(
        "https://lunary.app/api/og/crystal?date=2025-11-19",
        "utf8"
      ).toString("base64url")}&format=png`,
    ]);
  });

...
const mapPlatformsForAyrshare = (platforms: string[]): string[] => {
  return platforms.map(
    (platform) => INTERNAL_TO_AYRSHARE_PLATFORM[platform] || platform
  );
};
...
export const handleStandardPost = async (postData: PostData) => {
  ...
  logPostWorkflow("Standard Post Started", postData, "started", undefined, {
    requestId,
  });
  ...
  const hasTwitter =
    mappedPlatforms.includes("twitter") || mappedPlatforms.includes("x");
    ...
    pinterestOptions: postData.pinterestOptions
      ? (() => {
          ...
          if (options.boardId && !/^\d+$/.test(options.boardId)) {
            console.warn(
              "⚠️ Pinterest boardId must be numeric. Removing non-numeric boardId:",
              options.boardId
            );
  ...
  console.log("🚀 SENDING TO AYRSHARE API:", {
    ...
    headers: {
      ...
      Authorization: headers["Authorization"]
        ? `Bearer ${headers["Authorization"].substring(7, 15)}...`
        : "missing",
      "Profile-Key": headers["Profile-Key"]
        ? `${headers["Profile-Key"].substring(0, 8)}...`
        : "none",
    },
    body: {
      ...cleanedBody,
  ...
  try {
    ...
    if (response.ok) {
          ...
          if (post.errors && Array.isArray(post.errors)) {
            post.errors.forEach((error: any) => {
              ...
              if (error.platform === "twitter" || error.platform === "x") {
                ...
                if (error.code === 272) {
                  console.error(
                    "🐦 TWITTER 272 ERROR: Twitter account authorization expired! You need to relink your Twitter account in Ayrshare dashboard."
                  );
                  console.error(
                    "🐦 SOLUTION: Go to https://app.ayrshare.com/social-accounts and relink your Twitter account"
                  );
                } else if (error.code === 403) {
                  console.error(
                    "🐦 TWITTER 403 ERROR: This usually means the Twitter account is not properly connected or lacks permissions"
                  );
                } else if (error.code === 401) {
                  console.error(
                    "🐦 TWITTER 401 ERROR: Twitter authentication failed - check if the account is still connected"
                  );
                } else if (error.code === 400) {
                  console.error(
                    "🐦 TWITTER 400 ERROR: Bad request - possibly duplicate content or invalid parameters"
                  );
                }
              }
            });
      ...
      console.log(
        "  - Success platforms:",
        successPlatforms.length > 0 ? successPlatforms : "None"
      );
    ...
    if (!response.ok) {
            ...
            post.errors.forEach((error: any) => {
              if (error.platform && error.message) {
                if (error.code === 272) {
                  ...
                  postErrors.push(
                    `INSTAGRAM: Media processing error. Instagram posts require images or videos. Please add media to your post or remove Instagram from selected platforms.`
                  );
                } else if (
                  error.code === 132 &&
                  (error.platform === "twitter" || error.platform === "x")
                ) {
                  postErrors.push(
      ...
      if (response.status === 400) {
        if (result.message && result.message.includes("media")) {
          throw new Error(
            `Media Error: ${result.message}. Please check that your media URLs are publicly accessible and in a supported format.`
          );
        }
      ...
      if (
        response.status === 400 &&
        result.posts?.[0]?.errors?.some((e: any) => e.code === 137)
      ) {
        logAyrshareOperation({
          timestamp: new Date().toISOString(),
          ...
          status: "error",
          error:
            "Duplicate content - Ayrshare prevents posting duplicate content within 48 hours",
          data: {
            httpStatus: response.status,
            duplicatePostId: result.posts?.[0]?.id,
            affectedPlatforms:
              result.posts?.[0]?.errors?.map((e: any) => e.platform) || [],
          },
          requestId,
        });

        throw new Error(
          "Duplicate content detected. Ayrshare prevents posting identical content within 48 hours to comply with social media platform policies."
        );
      }
...
export const handleReplyPost = async (postData: PostData, replyUrl: string) => {
  ...
  if (!detectedPlatform) {
    throw new Error(
      "Could not detect a supported platform from the provided URL."
    );
  }

  // Map internal platform to Ayrshare platform
  const ayrshareDetectedPlatform =
    INTERNAL_TO_AYRSHARE_PLATFORM[detectedPlatform] || detectedPlatform;
  ...
  if (!ayrsharePostPlatforms.includes(ayrshareDetectedPlatform)) {
    throw new Error(
      `To reply, you must select the platform corresponding to the URL (${detectedPlatform}).`
    );
  }
  ...
  if (ayrshareDetectedPlatform === "twitter") {
    ...
    if (!postId)
      throw new Error(
        `Could not extract Post ID from the URL for ${detectedPlatform}.`
      );
    ...
    for (const platform of successfulPlatforms) {
      ...
      if (platformPostId) {
        ...
        commentPromises.push(
          handleCommentPost(commentData, platformPostId, platform)
        );
      }
    }
  }

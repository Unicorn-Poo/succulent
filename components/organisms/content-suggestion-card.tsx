      ...
      <div className="p-4">
        {isEditing ? (
          <textarea
            ...
            className="w-full p-3 border border-border rounded-lg resize-none focus:ring-2 focus:ring-brand-seafoam focus:border-transparent"
            rows={6}
        ...
        {hashtags.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1">
            {hashtags.map((tag, i) => (
              <span
                key={i}
                className="text-xs px-2 py-1 bg-brand-mint/10 dark:bg-brand-seafoam/20 text-brand-seafoam dark:text-brand-mint rounded-full"
              >
                #{tag}
              </span>
            ))}

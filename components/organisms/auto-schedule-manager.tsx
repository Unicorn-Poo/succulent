...
export default function AutoScheduleManager({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default: return 'bg-brand-mint/20 dark:bg-brand-seafoam/20 text-brand-seafoam dark:text-brand-mint';
    }
  };
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="p-4 bg-brand-mint/10 dark:bg-brand-seafoam/20 rounded-lg text-center">
              <p className="text-3xl font-bold text-brand-seafoam dark:text-brand-mint">{metrics.totalRules}</p>
              <p className="text-sm text-brand-seafoam dark:text-brand-mint">Total Rules</p>
            </div>
          ...
          {metrics.nextScheduledPost && (
            <div className="p-4 bg-brand-mint/10 dark:bg-brand-seafoam/20 border border-brand-mint/40 dark:border-brand-seafoam/40 rounded-lg">
              <h4 className="font-medium text-brand-seafoam dark:text-brand-mint mb-2">Next Scheduled Post</h4>
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-brand-seafoam">
      ...
      {activeTab === 'settings' && (
        <div className="space-y-6">
          ...
          <div className="p-4 border border-border rounded-lg">
            ...
            <div className="flex flex-wrap gap-2">
              {optimalTimes.map(time => (
                <span key={time} className="px-3 py-1 bg-brand-mint/20 dark:bg-brand-seafoam/20 text-brand-seafoam dark:text-brand-mint rounded-full text-sm">
                  {time}
                </span>
              ))}
            </div>

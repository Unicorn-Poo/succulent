              ...
              .map(item => (
                <div
                  ...
                  className={`p-4 border rounded-lg ${
                    selectedContent.includes(item.id) ? 'border-brand-seafoam bg-brand-mint/10 dark:bg-brand-seafoam/20' : 'border-border'
                  }`}
                >
              ...
              <div className="mb-3">
                <div className="flex flex-wrap gap-2">
                  {topic.hashtags.map(hashtag => (
                    <span key={hashtag} className="px-2 py-1 bg-brand-mint/20 dark:bg-brand-seafoam/20 text-brand-seafoam dark:text-brand-mint rounded text-sm">
                      {hashtag}
                    </span>
                  ))}
                </div>
      ...
      {activeTab === 'scheduled' && (
        <div className="space-y-4">
          {discoveredContent
            .filter(item => item.isScheduled)
            .map(item => (
              <div key={item.id} className="p-4 border rounded-lg bg-brand-mint/10 dark:bg-brand-seafoam/20">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-medium text-foreground">{item.title}</h4>
